...
Kullandığı kanallar: ${spamDedection.map(x => x.kanal).join(",")}\`\`\``)
      ...
      if(TalentPerms) {
        ...
        if (!uye) return message.channel.send(`${cevaplar.prefix} ${rolismi} rolü verebilmem için lütfen bir üyeyi etiketle __Örn:__ \`${sistem.prefix}${komutcuklar} @sehira/ID\`!`).then(x => x.delete({timeout: 5000}));
