...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    if(ayarlar.taglıalım != false && !uye.user.username.includes(ayarlar.tag) && !uye.user.username.includes(ayarlar.etiketTag) && !uye.roles.cache.has(roller.boosterRolü) && !uye.roles.cache.has(roller.vipRolü) && !message.member.hasPermission('ADMINISTRATOR') && !roller.kurucuRolleri.some(oku => message.member.roles.cache.has(oku))) return message.channel.send(cevaplar.taglıalım).then(x => x.delete({timeout: 5000}));
